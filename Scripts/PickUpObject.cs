    ...
    public class PickUpObject : MonoBehaviour
    {
        ...
        void Update()
        {
            if(canpickup == true) // if you enter thecollider of the objecct
            {
                ...
                if(Input.GetButtonDown("Fire1")) // if you want to throw the object, can be any key
                {
                    ObjectIwantToPickUp.GetComponent<Rigidbody>().isKinematic = false; // make the rigidbody work again
                    ObjectIwantToPickUp.transform.parent = null; // make the object no be a child of the hands
                    ObjectIwantToPickUp.GetComponent<Rigidbody>().AddForce(myHands.transform.forward * pushForce); // add a force to the object to throw it
                    hasItem = false; }                
